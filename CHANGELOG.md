...
### Виправлено
- **Вирішено проблему з кешуванням коду Python (`.pyc` файли).** Попередня помилка `AttributeError` продовжувала виникати через те, що зміни в коді не застосовувалися. Кеш було очищено, що гарантує виконання актуальної версії коду після перезапуску.
- **Виправлено критичну помилку `AttributeError` під час захисту.** Помилка `AttributeError: 'Card' object has no attribute 'rank'` остаточно виправлена шляхом заміни `.rank` на `.value` в логіці перевірки можливості підкинути карти.
- **Виправлено логіку створення гри (`/new`).** Помилка, через яку гра не створювалася, була викликана використанням застарілих методів `get_current()`. Код було оновлено для коректного отримання даних про чат та користувача з об'єкта `message`.

## [1.2.5] - 2024-05-24

### Виправлено
- **Виправлено критичну помилку `AttributeError` під час захисту.** Помилка `AttributeError: \'Card\' object has no attribute \'rank\'` виникала через неправильне ім\'я атрибута в логіці перевірки можливості підкинути карти (`attacker_can_continue`). Заміна `.rank` на `.value` відновила роботу гри.
- **Усунуто дублююче сповіщення про перехід ходу.** Видалено зайвий код, який надсилав нав\'язливе повідомлення "Хід гравця" з кнопкою "Атакувати", що погіршувало ігровий досвід. Тепер гра використовує єдину коректну систему сповіщень.
