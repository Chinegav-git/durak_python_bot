...
### Додано
- **Автовидалення повідомлення про атаку:** Інтерактивне повідомлення "підкинув(ла) карту" тепер автоматично видаляється через 7 секунд після того, як захисник успішно побив карту. Це зменшує кількість застарілих повідомлень у чаті.
- **Автовидалення повідомлення про пас:** Повідомлення "Пас!..." тепер автоматично видаляється через 5 секунд після відправки, щоб зменшити кількість повідомлень у чаті.
...
### Змінено
- **Покращено відображення карт:** Змінено текстове представлення карт. Тепер номінал (6-10, J, Q, K, A) та іконка масті розділені пробілом (наприклад, `10 ♥`), що покращує читабельність.
...
### Виправлено
- **Порядок повідомлень при пасі:** Гарантовано, що повідомлення "Пас!..." надсилається до того, як відбувається фактична передача ходу. Це забезпечує правильну послідовність подій для гравців.
- **Виправлено функціональність інлайн-кнопок:** Відновлено роботу кнопок "Приєднатися" та "Почати гру". Для цього створено нові обробники `join_inline.py` та `start_inline.py`, які коректно обробляють `callback_query`, що генеруються цими кнопками. Це усунуло проблему, що виникла після архітектурних змін.
...
### Документація
- **Оновлено структуру проекту:** Додано опис для файлів `durak/handlers/game/join_inline.py` та `durak/handlers/game/start_inline.py`, які відповідають за обробку інлайн-кнопок лобі.
