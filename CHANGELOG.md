...
### Fixed
- **Виправлено критичну помилку `RuntimeWarning: coroutine ... was never awaited`:** Усунуто серію помилок, які призводили до некоректної роботи та потенційних збоїв через відсутність `await` при виклику асинхронних функцій.
    - **Що змінено:**
        1.  В обробниках команд `leave.py`, `kill.py` та `global_leave.py` додано ключове слово `await` до викликів асинхронних методів `gm.end_game()` та `gm.get_game_from_chat()`.
        2.  Виправлено логічну помилку в `leave.py`, яка могла призвести до відправки повідомлення про наступний хід *після* фактичного завершення гри. Тепер при недостатній кількості гравців гра коректно завершується, і жодних зайвих повідомлень не надсилається.
    - **Чому це важливо:** Це виправлення забезпечує стабільну та передбачувану роботу ігрових команд, усуваючи "тихі" помилки, які могли порушити ігровий процес та викликати несподівану поведінку, особливо в сценаріях завершення гри.
