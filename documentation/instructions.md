...
2. **Завершение задачи и документирование.** Любая задача (добавление функции, исправление ошибки, рефакторинг) считается выполненной только после выполнения двух следующих шагов:
...
*   **`Pony ORM 0.7.19`**: Мощная Object-Relational Mapper для работы с базой данных SQLite. Используется для хранения и управления настройками и статистикой пользователей (`UserSetting`).
...
1.  **Создайте новый файл** в директории `durak/objects/decks/` (например, `my_theme.py`).
