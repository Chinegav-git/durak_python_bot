...
2. **Завершение задачи и документирование.** Любая задача (добавление функции, исправление ошибки, рефакторинг) считается выполненной только после выполнения двух следующих шаков:
    а) **Обновление документации:** Убедись, что все изменения в логике или структуре проекта отражены в соответствующей документации (например, в этом файле, `instructions.md`).
    б) **Обновление `CHANGELOG.md`:** Внеси краткую и ясную запись о всех сделанных изменениях в файл `CHANGELOG.md`. **Это — финальное действие**, завершающее работу над задачей.
        **Правило ведения журнала изменений**: Записи должны быть техническими и конкретными. Вместо описания пользовательской функции (например, "Добавили классный режим"), описывайте суть изменения в коде (например, "Рефакторинг: разделена логика состояния игры (`is_game_active`) и настроек (`display_mode`)"). Это критически важно для будущего анализа и отладки.
3. Для сложных правок, затрагивающих несколько файлов или содержащих комплексную логику, сначала формируй и представляй план действий. Приступай к реализации только после его одобрения. Это обеспечит атомарность и взвешенность твоих правовок.
4. Соблюдай стиль кода. Весь генерируемый или изменяемый тобой код на Python должен быть отформатирован с помощью `black` и проходить проверку линтером `flake8` без ошибок.
...
*   **Як використовувати:**
    ...
    *   Щоб змінити режим, вкажіть його назву як аргумент. Наприклад: `/gamemode text_and_sticker`
