...
6.  **Принцип финальной само-верификации:** Перед тем как объявить задачу выполненной, я обязан провести внутреннюю проверку по следующему чеклисту, чтобы гарантировать качество и полноту работы:
    ...
    *   `[ ]` **Структура проекту оновлена?**: Якщо я додавав, видаляв або перейменовував файли, чи оновив я відповідні описи в розділі "Структура проекта" файлу `documentation/instructions.md`?
...
8.  **Суворий стиль CHANGELOG.md:** При оновленні файлу `CHANGELOG.md` я зобов'язаний суворо дотримуватися стилю, тону та рівня деталізації попередніх записів. Цей файл є технічним логом для розробників, і його записи повинні бути максимально конкретними.
    ...
    **Мої записи в `CHANGELOG.md` повинні:**
    ...
    4.  **Історія змін є незмінною:** Після того, як версія була зафіксована та відправлена до репозиторію, її запис у `CHANGELOG.md` не можна змінювати. Це гарантує, що історія змін залишається надійним джерелом інформації.
...
9.  **Наслідування стилю за аналогією:** Перед створенням нового файлу або додаванням значних блоків коду в існуючий, я зобов'язаний спочатку знайти та проаналізувати **схожий за функцією та структурою код чи документ** у проекті.

    Моя мета — зрозуміти та відтворити існуючі патерни, стиль та конвенції. Це стосується:

    *   **Структури файлів:** Як організовані імпорти, функції та класи в аналогічних модулях (наприклад, при створенні нового обробника в `durak/handlers/`).
    *   **Стилю коду:** Як іменуються змінні та функції, який рівень коментування, як оформлюються docstrings.
    *   **Формату документації:** Як структуруються заголовки, списки та приклади коду в файлах `.md`.

    **Застосування на практиці:**
    *   **Завдання:** "Додай команду `/stats_reset`".
    *   **Мої дії за новим правилом:**
        1.  Я знаходжу існуючий обробник статистики (`durak/handlers/info/stats.py`).
        2.  Я аналізую його: як він отримує користувача, як працює з базою даних, як відповідає на повідомлення.
        3.  Я створюю новий функціонал, наслідуючи цей стиль, що забезпечує консистентність коду.

10. **Принцип повноти виправлень:**

    Коли я вношу зміни в певну одиницю функціоналу (наприклад, логіку старту гри, виходу з неї, перевірку статистики), я зобов'язаний знайти **всі місця в коді**, де ця функціональність реалізована, і застосувати зміни до кожного з них.

    Це гарантує, що поведінка бота буде послідовною, незалежно від того, як користувач ініціював дію (через команду, інлайн-кнопку, інлайн-режим тощо).

    **Практичний приклад застосування:**

    *   **Завдання:** "Зміни повідомлення про помилку при старті гри."
    *   **Мої дії за новим правилом:**
        1.  Я ставлю собі питання: "Якими способами користувач може розпочати гру?"
        2.  Я знаходжу перший шлях: команда `/start`. Це веде мене до файлу `durak/handlers/game/start.py`.
        3.  Я знаходжу другий шлях: інлайн-кнопка "Почати гру". Це веде мене до файлу `durak/handlers/game/start_inline.py`.
        4.  **Висновок:** Я розумію, що будь-які зміни в логіці або текстах, пов'язаних зі стартом гри, мають бути синхронно внесені в **обидва** ці файли.

## Архітектурні принципи та правила взаємодії

Цей розділ фіксує ключові технічні та організаційні правила для підтримки стабільності та передбачуваності проекту.

### Принцип трьох рівнів дій

Для розмежування повноважень асистента встановлюються три рівні доступу до дій:

*   **Рівень 1: Аналіз та читання (Повна автономія)**
    *   **Дозволені інструменти:** `read_file`, `list_files`.
    *   **Опис:** Асистент може вільно виконувати ці дії для аналізу проекту, пошуку інформації та розуміння контексту без додаткових запитів.

*   **Рівень 2: Модифікація проекту (Запропонувати, підтвердити, діяти)**
    *   **Дозволені інструменти:** `write_file`, `delete_file`.
    *   **Опис:** Асистент готує повний вміст файлів для зміни (`write_file`) або перелік файлів для видалення (`delete_file`) і **презентує їх для остаточного підтвердження**. Лише після вашого схвалення він виконує фактичний запис або видалення.

*   **Рівень 3: Зовнішні та критичні дії (Запитати, потім діяти)**
    *   **Дозволені інструменти:** `run_terminal_command`, `classic_firebase_hosting_deploy`.
    *   **Опис:** Асистент **категорично зобов'язаний** спочатку запропонувати план, що включає ці дії, і отримати явний дозвіл **перед** їх виконанням. Ці дії безпосередньо впливають на середовище виконання або можуть мати незворотні наслідки.

### Патерн взаємодії з Базою Даних (Запобігання `TransactionError`)

*   **Проблема:** Критична помилка `pony.orm.core.TransactionError: Another greenlet is trying to use the same transaction` виникає, коли асинхронна операція (`await`) виконується всередині активної транзакції бази даних (`with db_session:`).
*   **Рішення:** Вся робота з базою даних повинна бути інкапсульована в окремих, коротких **синхронних функціях**. Основні асинхронні обробники (`async def`) повинні спочатку викликати ці синхронні функції для роботи з БД, і лише **після цього** виконувати будь-які `await` операції (наприклад, `message.answer`).
...
*   **Тип додатку:** Цей проект є **серверним додатком** і вимагає для роботи постійно запущеного Python-процесу.
*   **Заборона ініціативи:** Асистенту **категорично заборонено** ініціювати або пропонувати розгортання проекту. Це рішення завжди залишається за розробником.
...
Кожен коміт, що завершує певну задачу (наприклад, виправлення помилки або додавання функціоналу), повинен бути пов'язаний з версією в `CHANGELOG.md`.

1.  **Повідомлення коміту повинно містити версію:** Повідомлення коміту повинно починатися з номера версії, взятого з `CHANGELOG.md`. Це створює прямий зв'язок між кодом та його документацією.

    **Приклад:**
    '''bash
    git commit -m "[1.2.15] Fix: Усунуто критичні помилки в /gamemode"
    '''

2.  **Використання тегів (опціонально, але рекомендовано):** Для фіксації значних релізів можна використовувати теги Git.

    **Приклад:**
    '''bash
    git tag -a v1.2.15 -m "Реліз версії 1.2.15, що включає виправлення для /gamemode"
    '''

### Відправка змін

При відправці змін в віддалений репозиторій, якщо ім'я локальної гілки відрізняється від імені віддаленої, використовуйте наступну команду:

'''bash
git push origin HEAD
'''

Ця команда відправить вашу поточну гілку (`HEAD`) на віддалений сервер (`origin`), створивши там гілку з таким же ім'ям.
