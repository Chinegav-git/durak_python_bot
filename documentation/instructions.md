...
Проект використовує два типи сховищ для різних завдань:

#### 1. Redis (для стану гри)

*   **Призначення:** Redis використовується як **основне сховище для стану активних ігор**. Кожен екземпляр класу `Game` серіалізується і зберігається в Redis за унікальним ключем (ID чату).
*   **Чому це важливо:**
    1.  **Надійність:** Ігри не втрачаються при перезапуску або падінні бота. Стан гри може бути відновлений у будь-який момент.
    2.  **Масштабованість:** Архітектура дозволяє запускати декілька екземплярів бота (воркерів), які працюватимуть з єдиним сховищем стану, що є ключовим для високих навантажень.
    3.  **Безвідмовність:** Бот стає "stateless", оскільки не зберігає критичні дані в оперативній пам'яті.

#### 2. SQLite (для налаштувань та статистики)

*   **Призначення:** Легковісна реляційна база даних для зберігання довготривалих налаштувань та статистики, які не змінюються щосекунди.
*   **ORM:** Взаємодія з базою даних відбувається через `Pony ORM`, що спрощує запити та керування схемою.
*   **Запланований рефакторинг:** Поточна реалізація використовує синхронну бібліотеку Pony ORM через `asyncio.to_thread`. Це є тимчасовим рішенням, яке створює ризики стану гонитви (race conditions). **Заплановано повний перехід на асинхронну ORM (наприклад, Tortoise ORM)**, щоб забезпечити повну асинхронність, надійність та атомарність операцій з базою даних.
...
База даних, керована через `Pony ORM`, містить дві основні таблиці:

1.  **`UserSetting`** (зберігає налаштування та статистику для кожного унікального користувача):
    *   `id` (Primary Key): Унікальний ID користувача Telegram.
    *   `stats` (bool): Прапорець, що контролює збір статистики для користувача.
    *   `is_playing` (bool): Прапорець, що вказує, чи бере користувач участь у грі зараз.
    *   `first_places` (int): Загальна кількість перемог.
    *   `games_played` (int): Загальна кількість зіграних ігор.
    *   `cards_played` (int): Загальна кількість зіграних карт.
    *   `cards_beaten` (int): Загальна кількість побитих карт.
    *   `cards_atack` (int): Загальна кількість карт, використаних для атаки.

2.  **`ChatSetting`** (зберігає налаштування для кожного унікального чату):
    *   `id` (Primary Key): Унікальний ID чату Telegram.
    *   `display_mode` (str): Режим гри (`text`, `text_and_sticker`).
    *   `card_theme` (str): Назва обраної теми карт (`classic`, `gold_trumps`).
    *   `is_game_active` (bool): Прапорець, що вказує, чи йде зараз гра в цьому чаті.
    *   `sticker_id_helper` (bool): Прапорець, чи ввімкнено режим допомоги для отримання ID стікерів (доступно лише для адміністраторів).
...
*   `durak/`
        ...
        *   `game/`
            ...
            *   `process_chosen.py`
                ...
                *   **Логіка:**
                    ...
                    *   **Спочатку отримує об'єкт `game` через `GameManager`, а потім** викликає відповідну асинхронну функцію з `durak/logic/actions.py` (наприклад, `do_attack_card`), **передаючи їй отриману гру**.
    ...
    *   `logic/`
        ...
        *   `actions.py`
            ...
            *   **Ключові функції:**
                ...
                *   `do_attack_card(game, player, card)`: Обробляє дію атаки. Змінює стан гри через об'єкт `Game`, оновлює статистику та надсилає повідомлення/стікер з атакуючою картою.
                *   `do_defence_card(game, player, atk_card, def_card)`: Обробляє дію захисту. Аналогічно до атаки, змінює стан, оновлює статистику, видаляє повідомлення про атаку та перевіряє, чи не завершено раунд, щоб викликати `do_turn`.
                *   `do_draw(game, player)`: Обробляє дію "взяти карти". Гравець `player` (захисник) забирає карти з поля (`game.take_all_field()`), після чого хід передається далі (`do_turn` з `skip_def=True`).
                *   `do_pass(game, player)`: Обробляє пас від атакуючого. Встановлює `game.is_pass = True` і, якщо всі карти побиті, завершує раунд, викликаючи `do_turn`.
            ...
            *   **Взаємодія:**
                ...
                *   **Примітка:** Ключові функції тепер **вимагають явну передачу об'єкта `game`**. Це робить потік даних прозорим і відповідає новій архітектурі без циклічних залежностей.
        *   `game_manager.py`
    ...
    *   `objects/`
        ...
        *   `player.py`
            ...
            *   **Основні методи:**
                *   `playable_card_atk(game)`, `playable_card_def(game, atk_card)`: Визначають, якими картами гравець може ходити. **Приймають об'єкт `game` як аргумент**.
                *   `can_beat(atk_card, def_card)`: Перевіряє, чи може одна карта побити іншу.
                *   `draw_cards_from_deck(game)`: Добирає карти з колоди. **Приймає об'єкт `game` як аргумент**.
            *   **Взаємодія:**
                *   **Керується:** Класом `Game`, який створює та керує списком об'єктів `Player`.
                *   **Примітка:** Клас більше не зберігає прямого посилання на гру; об'єкт гри тепер передається в його методи як аргумент, що усуває циклічні залежності.
        *   `decks/`
            *   `__init__.py`
            *   `classic.py`
            *   `gold_trumps.py`
