...
### Задача: Восстановление игрового цикла и локализации (✅ Завершено / Completed)

**Проблема:** Бот падает с ошибками `AttributeError: type object 'Card' has no attribute 'from_str'` и `TelegramBadRequest: message can't be deleted`. Причина в рассинхронизации между модулями, которые генерируют и парсят игровые действия, и в игнорировании системы локализации.

**План:**

-   [x] **Шаг 1: Восстановление `Card` (Файл: `durak/objects/card.py`)**
    -   **Действие:** Реализовать универсальный метод `@classmethod def from_str(cls, string: str)`, который будет понимать как технический формат (`6_c`), так и визуальный (`6 ♣`).
    -   **Цель:** Устранить `AttributeError` и заложить основу для парсинга на любом языке.

-   [x] **Шаг 2: Рефакторинг с учетом локализации (Файлы: `*.json`, `result.py`, `actions.py`)**
    -   **Действия:**
        1.  Добавить ключи для игровых действий (`inline.attack_action` и т.д.) в файлы локализации `durak/locales/*.json`.
        2.  Изменить `durak/logic/result.py` для использования `t('inline.attack_action', card=str(card))` при генерации инлайн-сообщений.
        3.  Переписать обработчик в `durak/handlers/game/actions.py`, чтобы он сравнивал входящий текст с локализованными шаблонами, а не с жестко закодированными строками.
    -   **Цель:** Обеспечить корректную обработку ходов на всех поддерживаемых языках (`uk`, `ru`, `en`).

-   [x] **Шаг 3: Исправление падений (Файл: `durak/handlers/game/actions.py`)**
    -   **Действие:** Обернуть вызов `message.delete()` в `try-except`.
    -   **Цель:** Предотвратить падение бота, если у него нет прав на удаление сообщения.
