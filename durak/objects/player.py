...
class Player:
    ...
    def playable_card_atk(self) -> List[Card]:
        ...
        for card in self.cards:
            # For current_player (attacker): check allow_atack limit
            # For support_player (third player): can always throw if field matches and limit not reached
            if game.current_player == self:
                if not game.allow_atack:
                    return []
            elif game.support_player == self:
                if not game.allow_support_attack:
                    return []
    ...
    def can_beat(self, atk_card: Card, def_card: Card) -> bool:
        if def_card.suit == self.game.trump:
            return ( atk_card.suit != self.game.trump ) or ( int(str(def_card.value)) > int(str(atk_card.value)) )
        
        elif ( def_card.suit == atk_card.suit ):
            return int(str(def_card.value)) > int(str(atk_card.value))
    ...
    def remove_card(self, card: Card):
        if card in self.cards:
            ...
            # Card not found in hand - this shouldn't happen in normal gameplay
            self.logger.warning(f"Attempted to remove card {card} not in player's hand")
